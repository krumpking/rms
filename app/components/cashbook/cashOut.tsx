	...
	const addCash = async () => {
		if (files.length > 0) {
			...
			try {
				...
				await uploadFile(
					`${adminId}/${CASHBOOOK_STORAGE_REF}/${name}`,
					files[0]
				);
				...
				// Thumbnail
				await uploadFile(
					`${adminId}/${CASHBOOOK_STORAGE_REF}/thumbnail_${name}`,
					compressedFile
				);
				...
				addDocument(CASHBOOOK_COLLECTION, transaction)
					...
					.catch((e) => {
						toast.error(
							'There was an error adding transaction, please try again'
						);
						console.error(e);
					});
			} catch (e) {
			...
			toast.error(
				'Hmmm looks like you did not add the cash out receipt/invoice/signature picture.'
			);
		}
	};
				...
				{loading ? (
					...
					<div className='bg-white rounded-[30px] p-4  grid grid-cols-2 gap-4'>
						<div className='flex flex-col'>
							...
							<div
								className='flex flex-col border-dashed border-2 border-[#8b0e06] place-items-center p-4 mb-6'
								{...getRootProps()}
							>
										...
										<Disclosure>
											...
											<Disclosure.Panel>
												<div className='flex flex-col shadow-xl p-4 rounded-[25px]'>
													...
													<ShowImage
														src={`/${adminId}/${CASHBOOOK_STORAGE_REF}/${v.file.thumbnail}`}
														alt={'Payment File'}
														style={'w-full h-64'}
													/>
												</div>
