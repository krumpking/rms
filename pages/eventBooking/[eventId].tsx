...
import {
  ...
  updateBooking,
  updateBookingEvent,
} from '../../app/api/bookingsApi';
import Pill from '../../app/components/pill';
import { IAttendee, IBookingEvent } from '../../app/types/reservationTypes';
...
const EventBooking = () => {
  ...
  const [bookingTopics, setBookingTopics] = useState<any[]>([
    'Name',
    'Email',
    'Phone Number',
  ]);
  ...
  const addBooking = () => {
    ...
    if (name !== '' && phone !== '' && email !== '') {
      ...
      addBookingToEvent(docId, booking)
        .then((v) => {
      ...
      toast.error(
        `Name, Phone and Email cannot all be empty, please try again`
      );
      setLoading(false);
    }
  };

  const addBookingDataFromTable = (
    name: string,
    phone: string,
    email: string
  ) => {
  ...
  const deleteBooking = (editMember: any) => {
    ...
    if (confirm(text) == true) {
      ...
      updateBooking(docId, { bookings: booking })
        .then((v: any) => {
          getBookingEvent(docId);
          setLoading(false);
        })
  ...
  const AddEvent = () => {
    ...
    if (
      ...
      updateBookingEvent(docId, booking)
        .then((r) => {
          setLoading(false);
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {edit ? (
                      <>
                        <div className="mb-6">
                          <p className="text-center text-xs text-gray-300 mb-1 font-bold">
                            Start Date
                          </p>
                          <input
                            type="date"
                          ...
                          {bookings.map((value, index) => {
                            return (
                              ...
                              >
                                ...
                                <td>{getData(value.phone)}</td>
                                <td>
                                  {value.attended ? 'Attended' : 'Not Attended'}
                                </td>
                                <td className=" whitespace-nowrap text-right">
                                  <Menu>
                                    {({ open }) => (
            ...
            >
              <div className="bg-slate-100 my-8 inline-block w-full max-w-md transform overflow-hidden rounded-2xl p-6 text-left align-middle shadow-xl transition-all">
                <div className="flex flex-col space-y-2 ">
                  ...
                  <div>
                    {typeof notesMember !== 'undefined' ? (
                      notesMember.notes.map((v: any) => {
                        return (
                          <p className="p-4 shadow-2xl rounded-[20px]" key={v}>
                            {getData(v)}
                          </p>
                        );
                      })
