	...
	const sendStockItems = async () => {
		...
		if (stockItem.itemNumber - numberOfItems > 0) {
			...
			let newItem = {
				...stockItem,
				dateOfUpdate: new Date().toDateString(),
				itemNumber: stockItem.itemNumber - numberOfItems,
			};
			//Logic to delete the item
			updateDocument(STOCK_ITEM_COLLECTION, stockItem.id, newItem)
				.then((v) => {})
				.catch((e: any) => {
					console.error(e);
			...
			addDocument(STOCK_ITEM_COLLECTION, addItem)
				.then((r) => {
					getStockItems();
				})
				.catch((e) => {
			...
			toast.error(
				`Looks like there is only ${stockItem.itemNumber} of ${stockItem.title} left and you are trying to move ${numberOfItems}.Try again!!`
			);
		}
	};
			...
			<div className='bg-white rounded-[30px] p-4 '>
				{loading ? (
					...
					<AppAccess access={access} component={component}>
						<div className='flex flex-col max-h-[700px] w-full gap-4 p-4'>
							<div className=''>
								<input
									type='text'
									value={search}
				...
				>
					<div className='min-h-screen px-4 text-center backdrop-blur-sm '>
                        ...
                        '
									...
									<div className='flex flex-row space-x-4 text-left'>
										...
										/>
										<p>
											Throw {numberOfItems} of {stockItem.title} into the bin
										</p>
									</div>
									<button
										onClick={() => {
                   ...
                   '
									>
										{bin
											? 'Confirm Bin'
											: `Confirm Move ${numberOfItems} of ${stockItem.title}`}
									</button>
								</div>
							</div>
						</Transition.Child>
